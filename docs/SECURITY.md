...
The current implementation stores PIT credentials in the local fallback credential file and supports explicit read-only network calls for PIT validation, raw GET, and typed CRM reads across locations, contacts, conversations, pipelines, and opportunities.
